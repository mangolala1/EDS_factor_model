...
def compute_exposures_for_quarter(
    ...
    # OVERRIDE QUARTER RANGE FOR DEBUGGING SMALL WINDOWS
    # quarter_start_dt = pd.to_datetime(start_date)
    ...
    # ADDED
    # Index FULL lookback data too (needed for warm-up)
    all_prices_idx = all_prices.set_index('DATE', drop=False)
    all_returns_idx = all_returns.set_index('DATE', drop=False)
    # ******
    ...
    # (B) output dates = only what you want to SAVE (e.g., 5 days)
    output_dates = [d for d in trading_dates if start_dt <= pd.to_datetime(d) <= end_dt]
    if len(output_dates) == 0:
        return {'processed': 0, 'skipped': 0, 'quarter': f"{year}Q{quarter}", 'csv_path': None}

    # (A) warm-up dates = dates BEFORE start_date (use lookback data you already loaded)
    # warmup_dates = sorted(
    #     all_returns[
    #         (all_returns['DATE'] >= lookback_start_dt) &
    #         (all_returns['DATE'] < start_dt)
    #     ]['DATE'].unique()
    # )

    warmup_dates = sorted(
        set(all_returns.loc[(all_returns['DATE'] >= lookback_start_dt) & (all_returns['DATE'] < start_dt), 'DATE'].unique())
        & set(all_prices.loc[(all_prices['DATE'] >= lookback_start_dt) & (all_prices['DATE'] < start_dt), 'DATE'].unique())
    )
    # *******

    # from here onward, process only the output dates
    trading_dates = output_dates
    ...
    # WARM-UP: populate ring buffers using prior dates (no exposures saved)
    if len(warmup_dates) > 0:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] {year}Q{quarter}: Warming up buffers with {len(warmup_dates)} prior dates...")
        for date_T in warmup_dates:
            try:
                prices_T  = all_prices_idx.loc[[date_T]].copy() if date_T in all_prices_idx.index else pd.DataFrame()
                returns_T = all_returns_idx.loc[[date_T]].copy() if date_T in all_returns_idx.index else pd.DataFrame()
            ...
            for row in returns_prices.itertuples():
                calculator_manager.update_stock(row.FACTSET_ID, date_T, row.ONE_DAY_PCT, row.DOLLAR_VOL)

    print(f"[{datetime.now().strftime('%H:%M:%S')}] {year}Q{quarter}: Done warm-up. Now computing exposures for {len(trading_dates)} output dates...")    
    ...
    print(f"... Buffers warmed up using {len(warmup_dates)} prior dates; now processing {len(trading_dates)} output dates.")
